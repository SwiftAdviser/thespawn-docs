...
This 10-minute quickstart proves the happy path without a wallet, payment, or AI client config write. You will search The Spawn, inspect Social Intel API, verify its MCP tool, run a demo request that does not require payment, and confirm the paid endpoint returns a machine-readable x402 challenge.

<Info>
Last verified: June 16, 2026. The canonical example is `base:29382`, Social Intel API.
</Info>
...
- one unpaid paid request returning HTTP `402` with x402 payment terms;
...
- Codex installed only if you want to preview the Codex MCP config in Step 6.
...
What just happened: MCP is the tool transport. `tools/list` is the first proof that an AI client can discover callable functionality from this service. This check comes before install because a valid URL can still expose no usable tools.
...
Checkpoint: the service returned useful preview data without a wallet or payment. This is the strongest first-run path for someone who is not crypto-native, because the result shape is visible before wallet policy becomes part of the flow.

## Step 5: Confirm the unpaid 402 contract
...
The JSON body should include `x402Version: 2`, `resource.url`, `accepts[]`, and at least one `amount` of `500000` base units. This proves the paid path explains price and accepted rails before a wallet signs anything. However, stop here unless you intentionally want to test payment with a funded wallet.

## Step 6: Preview install into Codex
...
```text
spwnr-social-intel-api
```

If the server is missing, keep the dry-run output and compare the config path against [MCP client setup](/mcp/client-setup).
