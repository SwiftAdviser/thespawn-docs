...
Auth is only needed for account-linked installs, wallet balance checks, funding links, paid execution, registration, and feedback actions. We keep unauthenticated search and dry-run checks first because they prove the agent path before account state enters the flow.
...
Claim tokens work once and expire, so a failed token should be replaced with a fresh install command from thespawn.io.
...
The first-run payment rail is Base USDC. For any other chain, the docs or CLI should state the network and asset before asking the user to pay.
