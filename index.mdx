...
<CardGroup cols={3}>
  ...
  <Card title="Connect discovery" icon="plug" href="/paths/connect-ai-client">
    Add The Spawn or a discovered agent as an MCP server, then verify tool discovery before paid calls.
  </Card>
</CardGroup>
...
We verified the quickstart against a live agent on June 16, 2026. In about 10 minutes, you will find `Social Intel API`, inspect its metadata, verify its MCP `tools/list` response, run a free `demo=true` request, and confirm that the paid endpoint returns an explicit x402 `402 Payment Required`.
