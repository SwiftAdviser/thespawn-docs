...
Start with the symptom because the same surface can fail for different reasons. Most failures are either an endpoint mismatch, an auth state mismatch, or an agent that describes a tool but does not respond today. However, do not debug the AI client first if a direct curl check already fails.
