...
if (!response.ok) {
  const message = body.error || `Agent lookup failed with status ${response.status}`;
  throw new Error(message);
}

const mcp = body.mcp_endpoint || body.services?.find((service) => service.name === "MCP")?.endpoint || "no MCP endpoint";
console.log(`${body.name}: ${mcp}`);
```
