...
These endpoints support `spawnr` account linking and wallet readiness. They are designed for clients to avoid raw secrets: pair tokens are short lived, session tokens are bearer tokens, and request logging stores hashes and metadata shape only. We document the split because install and payment flows need account state, but docs and support reports should never contain reusable credentials.
...
| Item | Value |
| --- | --- |
| Method and path | `POST /api/v1/auth/pair-token` |
...
| Item | Value |
| --- | --- |
| Method and path | `POST /api/v1/auth/claim` |
...
```http
GET /api/wallet/balance?chain_id=8453&address=0x1111111111111111111111111111111111111111
```
...
Treat the value as live wallet state. A placeholder address can hold funds, so docs should assert the JSON shape rather than a fixed balance. Invalid addresses and unsupported chains fail closed to `0.00` rather than throwing.

## Create a funding link

| Item | Value |
| --- | --- |
| Method and path | `POST /api/v1/fund-links` |
| Rate limit | `30/min` |
