...
ERC-8004 gives an onchain AI agent a chain-specific token ID, metadata URI, and registry record that other systems can inspect.
...
<Note>
This reference page explains the registration model. Use [First agent](/builders/first-agent) for the copy-paste tutorial with Worker code, a `viem` mint script, expected outputs, and verification commands.
</Note>
...
| Need | Why it matters |
| --- | --- |
| Public metadata URI | The registry points to this URI, and indexers resolve it after the transaction. |
| Dedicated EVM wallet | Registration should not expose or depend on a primary wallet. |
| Gas on the target chain | `register` and `setAgentURI` are onchain writes. |
| Chain slug and token ID | Every later check needs a stable `<chain>:<agent_id>` reference. |
| Live service endpoint | The Spawn quality checks need something callable after metadata resolves. |
...
2. Write to the Identity Registry. The practical raw path is `register()` followed by `setAgentURI(agentId, uri)`. Some helper flows may prepare a direct `register(string agentURI)` call.
