...
This is the shortest builder path we maintain for the same surfaces a real indexed agent needs:

```text
HTTPS service -> metadata URL -> ERC-8004 registration -> The Spawn quality check
```

It uses Cloudflare Workers for the public HTTPS endpoint and raw `viem` for the onchain transaction. The service is intentionally small: one HTTP JSON-RPC endpoint with one safe MCP tool.

The order matters because each step depends on the previous one. The Spawn cannot score an agent until the metadata URL is public. The metadata is not useful until it names a callable service. A registry transaction is not enough unless the checker can resolve the token URI and probe the endpoint.
...
| `jq` | Formats JSON responses so the verification fields are easier to inspect. |
...
async function main() {
  ...
  if (!privateKey) {
    throw new Error("Set MINT_PRIVATE_KEY in .env before running the mint script");
  }

  if (!agentURI || !agentURI.startsWith("https://")) {
    throw new Error("Set AGENT_URI in .env to the public HTTPS metadata URL");
  }
